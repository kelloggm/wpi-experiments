...
/**
 * A new entry is inserted into the cache, e.g. by {@link Cache#put(Object, Object)} or by read
 * through and the {@link CacheLoader}.
 *
 * @author Jens Wilke
 */
public interface CacheEntryCreatedListener<K, V> extends CacheEntryOperationListener<K, V> {
  ...
  /**
   * Called for the creation of a cache entry and after all cache writers ran successfully. A
   * synchronous event is executed before the entry becomes visible for cache clients. If an
   * inserted or loaded value expires immediately, no created event is sent.
   *
