...
/**
 * An invokable function to perform an atomic operation on a cache entry. The entry processor is
 * called via {@link org.cache2k.Cache#invoke} or {@link org.cache2k.Cache#invokeAll}.
 *
 * <p>With the entry processor it is possible to realize arbitrary operation semantics for an entry.
 * For example, the method {@link org.cache2k.Cache#replaceIfEquals} can be expressed with the entry
 ...
 * }</pre>
 *
 * <p>For effects on the loader and writer and further details consult the documentation on the
 * {@link MutableCacheEntry}
 *
 * @author Jens Wilke
...
public interface EntryProcessor<K, V, @Nullable R> extends DataAware<K, V> {
   ...
   * For one call to {@link org.cache2k.Cache#invoke} the method might be called several times. Some
   * methods of {@link MutableCacheEntry} throw exceptions that are consumed by the cache to do
   ...
   * @throws Exception an arbitrary exception that will be wrapped into a {@link
   *     EntryProcessingException}. If an exception happens, the cache content will not be altered.
   */
  @Nullable
