...
/**
 * Information of a cache for introspection at runtime, for generic monitoring and management
 * proposes. Additional information is available via {@link CacheStatistics} if statistics are
 * enabled.
 *
 * <p>It is intentionally that there is no way to retrieve the original configuration after
 * creation.
 *
 * @author Jens Wilke
 */
public interface CacheInfo {
  ...
  /**
   * The current number of entries within the cache, starting with 0. This value is an estimate,
  ...
  /**
   * Either {@link #getMaximumWeight()} or {@link #getEntryCapacity()} depending on whether a
   * weigher is present or not.
   *
   * @see Weigher
   */
  ...
  /**
   * A loader is configured. This can be used to determine, whether it makes sense to send the
   * report/monitor the loader metrics.
   */
  boolean isLoaderPresent();

  /**
   * A weigher is configured. This can be used to determine, whether it makes sense to send the
   * report/monitor the weigher metrics.
   */
   ...
   *
   * <p>We use the old type {@code Date} and not {@code Instant} here in order to make this
   * interface usable for JMX directly, which does not support {@code Instant}.
   */
  ...
  /**
   * Configured {@link Cache2kBuilder#expireAfterWrite(Duration)} or {@link Long#MAX_VALUE} if no
   * expiry.
   *
   * @since 2.6
   */
